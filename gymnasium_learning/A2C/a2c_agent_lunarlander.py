...
class Critic(nn.Module):
    def __init__(self, state_dim):
        ...
        self.fc1 = nn.Linear(state_dim, 256)
        self.fc2 = nn.Linear(256, 128)
        self.fc3 = nn.Linear(128, 1) # 输出状态价值 V(s)
...
# 3. 定义 A2C Agent
class A2CAgent:
    def __init__(self, state_dim, action_dim, lr_actor=0.0005, lr_critic=0.001, gamma=0.99, entropy_coef=0.01):
        self.gamma = gamma
    ...
    def update_networks(self, next_state, done):
        ...
        # 添加熵正则化项
        entropy = - (log_probs.exp() * log_probs).sum() # 策略熵
        actor_loss = actor_loss - self.entropy_coef * entropy

        # Critic Loss
        critic_loss = nn.MSELoss()(values.squeeze(), returns).sum()
...
def train():
    ...
    for episode in range(num_episodes):
        ...
        agent.update_networks(state, done)
